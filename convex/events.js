...
export const createEvent = mutation({
  args: {
    ...
    hasPro: v.optional(v.boolean()), // kept for backward compat, ignored server-side
  },
  handler: async (ctx, args) => {
    try {
      const user = await ctx.runQuery(internal.users.getCurrentUser);

      // SERVER-SIDE CHECK: Use database isPro, not client-passed value
      ...
      // Create event (exclude hasPro — it's not a schema field)
